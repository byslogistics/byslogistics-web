...
| [src/data_files/](src/data_files/)                   | [constants.ts](src/data_files/constants.ts) (`SITE`, `CONTACT`, `LEGAL`, `FORMS`, `ANALYTICS`, `SEO`, `OG`, `partnersData`), [sinonimos.ts](src/data_files/sinonimos.ts) (cómo llama el cliente a lo que el catálogo llama de otra manera), [empresa.ts](src/data_files/empresa.ts) (`RESENA`, `SECTORES`) y `faqs.json`. |
