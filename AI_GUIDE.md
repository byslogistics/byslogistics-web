...
- **Todo aparece en movimiento**, y casi nunca hay que escribirlo. Son tres piezas, y elegir mal la pieza es el error probable:
  ...
  - `data-anim-entrada`: la misma cascada pero **para lo que ya se ve al abrir la página** (el hero y los encabezados de página). Es CSS puro y arranca en el primer fotograma; la cascada, en cambio, la revela un observador de JavaScript, y arriba del todo eso deja el titular escondido unos milisegundos. **Sobre el pliegue va `entrada`, nunca `cascada`.**
