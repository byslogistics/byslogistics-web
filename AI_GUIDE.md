...
| [scripts/](scripts/)                                 | `extract-xlsx-images.mjs` (saca las fotos del listado de precios), `make-dark-logo.mjs` (genera las versiones del logo para fondo oscuro), `importar-resenas.mjs` (publica las reseñas de Google desde un volcado) y `smoke.mjs` (comprueba que las rutas responden).                                                                                                                                                |
...
- **Todo aparece en movimiento**, y casi nunca hay que escribirlo. Son tres piezas, y elegir mal la pieza es el error probable:
  - `data-anim-scope`, que MainLayout pone en `<main>`: hace que **toda `<section>` hija aparezca al entrar en pantalla**, sin tocar ninguna página. Quedan fuera la primera —está sobre el pliegue— y las que animan por dentro con una cascada.
  - `data-anim-cascada` en un contenedor: **escalona a sus hijos directos**, que es el gesto del cierre de la portada y el que la clienta quiso en todo el sitio. Admite `izquierda` y `derecha`. Un atributo por sección, no uno por renglón.
  - `data-anim-entrada`: la misma cascada —misma duración, misma curva, mismo escalón y mismo recorrido, así que el gesto es indistinguible— pero **para lo que ya se ve al abrir la página** (el hero y los encabezados de página). Es CSS puro y arranca en el primer fotograma; la cascada, en cambio, la revela un observador de JavaScript, y arriba del todo eso deja el titular escondido unos milisegundos. **Sobre el pliegue va `entrada`, nunca `cascada`.** También admite `izquierda` y `derecha`.

  Los tres comparten los mismos números —0,7 s, la curva `cubic-bezier(0.22, 1, 0.36, 1)`, 80 ms de escalón y 2,25 rem de recorrido—, y eso es a propósito: son un solo gesto repetido, no tres animaciones parecidas. Si se cambian, se cambian los tres.

  Para algo suelto queda `data-anim` a mano (`sube`, `izquierda`, `derecha`, `escala`, o `no` para sacar a una sección del automático) con `data-anim-retraso="1..6"`. Todo vive en el bloque «APARICIÓN AL HACER SCROLL» de [global.css](src/assets/styles/global.css); quien revela es [aparecer.js](src/assets/scripts/aparecer.js). Dos reglas que no se pueden romper: sin JavaScript no se esconde nada, y con `prefers-reduced-motion` el sistema entero no existe.
...
- **Reseñas de clientes** → NO se escriben a mano en [opiniones.ts](src/data_files/opiniones.ts): se dejan como volcado en `📤SUBIR-CONTENIDO/` y las publica `node scripts/importar-resenas.mjs`. El README de esa carpeta explica cómo sacarlas de la ficha (Google no ofrece descargarlas: su API devuelve cinco como mucho, así que se copian). Lo que se publique tiene que ser **lo que está en Google, tal cual** —sin arreglar la redacción ni dejar fuera la menos favorable—, porque cada tarjeta enlaza a la ficha y cualquiera puede cotejarla. Ahí no se escriben testimonios de redacción propia.
